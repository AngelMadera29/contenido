...
if ($op=="update"){
	if ($_POST["id"]!=''){
		...
		{
			comprobar_previsualizacion("administracion/dbvideos/".$nombre_de_video[0],"previsualizaciones","1");
		}
...
'".$canal."')";
 ...
 $id = $bbdd->resultado_id();
		...
		$nombre_de_video=explode('?',$video_id);
		
		if (file_exists("administracion/db/videos/".$nombre_de_video[0])&&$nombre_de_video[0]!="sin_imagen.jpg"&&$nombre_de_video[0]!="")
		{
			comprobar_previsualizacion("administracion/db/videos/".$nombre_de_video[0],"previsualizaciones","1");
		}
